...
function upsertReportLossDist(run_id, subset_on, payload) {
    ...
    db.prepare(''
        ...
        + '  at_epoch   = excluded.at_epoch, '
        + '  samples    = excluded.samples, '
        + '  created_at = excluded.created_at').run(run_id, subset_on, lossesJson, payload.note ?? null, payload.at_step ?? null, payload.at_epoch ?? null, samples, Date.now());
}
function getReportLossDist(run_id, subset_on) {
    try {
        const row = db.prepare('SELECT run_id, subset_on, losses, note, at_step, at_epoch, samples FROM report_loss_dist WHERE run_id=? AND subset_on=?').get(run_id, subset_on);
        if (!row)
            return null;
        let losses = [];
        try {
