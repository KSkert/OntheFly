...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('seamless.showDashboard', () => openPanel(context)));
...
function openPanel(context) {
    ...
    panel = vscode.window.createWebviewPanel('seamlessDashboard', 'Seamless ModelDev', vscode.ViewColumn.Active, {
        enableScripts: true,
...
function getHtml(context, webview, nonce) {
  ...
  `.trim();
    ...
    if (!chartUri)
        console.warn('[seamless] Chart.js not found at media/chart.umd.js or node_modules/chart.js/dist/chart.umd.js');
    if (!jsUri)
        console.warn('[seamless] dashboard.js not found. Buttons will not work.');
    if (!dagLayoutUri)
        console.warn('[seamless] dag_layout.js not found. DAG will fall back or render linearly.');
    return html;
}
function getNonce() {
    let text = '';
...
async function onMessage(context, m) {
    ...
    switch (m.command) {
        case 'chooseScript': {
            ...
            await ws.update("seamless.scriptPath" /* Keys.ScriptPath */, p);
        ...
        case 'setPython': {
            ...
            await ws.update("seamless.pythonPath" /* Keys.PythonPath */, chosen);
        ...
        case 'rewind':
            sendCtl({ cmd: 'rewind_steps', steps: Math.max(0, Number(m.steps || 0)) });
            break; // updated
        case 'saveCkpt':
            sendCtl({ cmd: 'save_ckpt' });
            break;
        case 'fork':
        ...
        case 'generateReport': {
            try {
                ...
                (0, storage_1.upsertReportLossDist)(String(runId), subset_on, { losses, note, at_step, at_epoch, samples: losses.length });
                post({
                    type: 'reportData',
                    run_id: String(runId),
            ...
            try {
                ...
                if (proc && proc.stdin.writable) {
                    ...
                    try {
                        ...
                        const scriptPath = context.workspaceState.get("seamless.scriptPath" /* Keys.ScriptPath */) || '';
...
async function startRun(context, mode) {
    ...
    const ws = context.workspaceState;
    const python = ws.get("seamless.pythonPath" /* Keys.PythonPath */)?.trim();
    const script = ws.get("seamless.scriptPath" /* Keys.ScriptPath */)?.trim();
    if (!python) {
        vscode.window.showErrorMessage('Set a Python interpreter first (gear icon).');
        return;
    }
...
function handleLine(line) {
    ...
    if (obj && obj.type === 'trainStep') {
        ...
        try {
            ...
            console.warn('[seamless] sqlite persist error for step:', e);
        }
        return;
    }
...
async function runDataExplorer(context, subArgs) {
    const ws = context.workspaceState;
    const python = ws.get("seamless.pythonPath" /* Keys.PythonPath */) || 'python';
    return new Promise((resolve, reject) => {
        const p = (0, child_process_1.spawn)(python, ['-m', 'seamless_modeldev.data_explorer', ...subArgs], { env: process.env });
        let out = '';
        let err = '';
