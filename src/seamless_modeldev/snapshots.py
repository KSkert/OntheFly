# # src/seamless_modeldev/snapshots.py
...
# class SnapshotRef:
#     """
#     Immutable reference to a model snapshot that a planner/worker can consume.

#     Attributes
#     ----------
#     owner : str
#         Run identifier (e.g., "baseline", "foo-auto@200").
#     version : int
#         Monotonic version number for that owner (usually the *training step* at
#         which the snapshot was taken).
#     token : str
#         Opaque freshness key. The session will *only* accept plans that carry a
#         token equal to the token of the snapshot it most recently published for
#         that owner. The token binds (owner, version, payload).
#     state_dict : Optional[Dict[str, torch.Tensor]]
#         CPU tensors representing the model parameters for this snapshot.
#         If None, the consumer must load from `path`.
#     path : Optional[str]
#         On-disk location of the snapshot (when spilled). If present and
#         `state_dict` is None, consumers should load from this path.
#     bytes_estimate : int
#         Approximate memory footprint of the stored state_dict, used for
...
#     SnapshotManager
#     ----------------
#     A small, thread-safe broker for *planning snapshots* that decouples
#     model-versioning from checkpoint files. It gives you:

#       • **Strict freshness** via opaque tokens (no timestamp/path heuristics).
#       • **Low latency** by handing the worker a CPU `state_dict` directly,
#         avoiding `torch.save`/`torch.load` in the hot path.
#       • **Backpressure-ready** intake—publish at a controlled cadence, but
#         still safe to coalesce/preempt later.
#       • **Optional spill-to-disk** (e.g., for large models or when memory is
#         constrained), while keeping the token contract the same.

#     Design notes
#     ------------
#     - Snapshots are keyed by (owner, version, token). The token is random
#       (UUID) so that even repeated “same version” publishes are distinguishable.
#     - We store a *tiny* per-owner ring (default keep=3). Each entry holds either
#       a CPU `state_dict` (fast path) or a `path` (spilled).
#     - This manager does *not* run any background threads. It is a plain,
#       lock-guarded dictionary with small, predictable critical sections.

#     Typical flow
#     ------------
#       session:
#         token, ref = snapshots.push_from_model(owner, version, model)
#         expected_token[owner] = token
#         feature_worker.submit_snapshot(owner=owner, version=version, token=token, snapshot=ref)

#       worker:
#         loads weights from ref.state_dict (if present) else from ref.path
#         computes features/plan, returns plan with the *same* token

#       session:
#         accepts the plan *only if* token == expected_token[owner]

#     API
#     ---
#     - push_from_model(owner, version, model) -> (token, SnapshotRef)
#     - push(owner, version, state_dict) -> (token, SnapshotRef)
#     - latest(owner) -> Optional[SnapshotRef]
#     - get(token) -> Optional[SnapshotRef]
#     - spill(token, dir_or_path) -> SnapshotRef
#     - drop_old(owner, keep)
#     """

#     def __init__(self, keep_per_owner: int = 3, default_spill_dir: Optional[str] = None):
#         self._keep = max(1, int(keep_per_owner))
#         self._spill_dir = default_spill_dir
#         self._by_owner: Dict[str, Dict[str, SnapshotRef]] = {}
#         self._order: Dict[str, list[str]] = {}  # per-owner token order (newest at end)
#         self._by_token: Dict[str, SnapshotRef] = {}
...
#         """
#         Capture a CPU `state_dict` snapshot from `model` and register it.
#         Non-blocking GPU→CPU copies are attempted where supported.
#         """
#         sd = model.state_dict()
#         cpu_sd: Dict[str, torch.Tensor] = {}
#         # Try to minimize synch points; fall back safely.
#         for k, t in sd.items():
#             if t.is_cuda:
#                 # Non-blocking copy when possible
...
#         """
#         Register a ready-made CPU state_dict as a snapshot for `owner@version`.
#         Returns (token, SnapshotRef).
#         """
...
#         """
#         Persist the snapshot to disk, updating the stored ref to point at the file.
#         If `dir_or_path` is a directory, we create a filename under it; if it is a
#         file path, we write exactly there. Returns the updated SnapshotRef.
#         """
#         with self._lock:
...
#                 return ref  # already spilled & released SD

#         # Decide the path (outside the lock)
...
#         blob = {"model": ref.state_dict}
#         torch.save(blob, path)

#         # Update the ref to drop SD and keep path (reacquire lock)
#         with self._lock:
#             new_ref = SnapshotRef(
#                 owner=ref.owner,
#                 version=ref.version,
...
#         """Manually trim the per-owner ring to `keep` newest snapshots."""
