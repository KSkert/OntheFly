...
class TrainMixin:
    ...
    Keeps the outer 'SeamlessSession' thin while preserving method names.
    """
    ...
    def serve(self, max_steps: Optional[int] = None, max_epochs: Optional[int] = None, do_test_after: bool = False):
        ...
        resume_run    = os.getenv("SEAMLESS_RESUME_RUN_ID") or None
        init_ckpt     = os.getenv("SEAMLESS_INIT_CKPT") or None
        init_step_env = os.getenv("SEAMLESS_INIT_STEP") or None
        ...
        if init_ckpt:
            if os.path.exists(init_ckpt):
                try:
                    ...
                    # Prefer explicit SEAMLESS_INIT_STEP when provided; otherwise trust ckpt contents.
