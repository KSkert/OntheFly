...
#  Gen Ep 10 | Train MSE: 0.014521 | Val MSE: 0.035652
#  Gen Ep 20 | Train MSE: 0.006573 | Val MSE: 0.013942
#  Gen Ep 30 | Train MSE: 0.003768 | Val MSE: 0.021439
#  Gen Ep 40 | Train MSE: 0.004720 | Val MSE: 0.030013
#  Gen Ep 50 | Train MSE: 0.002863 | Val MSE: 0.023403
#  Gen Ep 60 | Train MSE: 0.004080 | Val MSE: 0.017443
# Clustering 26 hard samples into 3 specialists...
# Training specialists...
#  Specialist 1/3 trained on 8 samples.
#  Specialist 2/3 trained on 8 samples.
#  Specialist 3/3 trained on 10 samples.
# Training gating network...
#  Gate Ep 1 | Val MSE: 0.028367
#  Gate Ep 2 | Val MSE: 0.028050
#  Gate Ep 3 | Val MSE: 0.027714
#  Gate Ep 4 | Val MSE: 0.027361
#  Gate Ep 5 | Val MSE: 0.026997
#  Gate Ep 6 | Val MSE: 0.026608
#  Gate Ep 7 | Val MSE: 0.026189
#  Gate Ep 8 | Val MSE: 0.025739
#  Gate Ep 9 | Val MSE: 0.025279
#  Gate Ep10 | Val MSE: 0.024755
#  Gate Ep11 | Val MSE: 0.024161
#  Gate Ep12 | Val MSE: 0.023506
#  Gate Ep13 | Val MSE: 0.022765
#  Gate Ep14 | Val MSE: 0.021929
#  Gate Ep15 | Val MSE: 0.020950
#  Gate Ep16 | Val MSE: 0.019810
#  Gate Ep17 | Val MSE: 0.018485
#  Gate Ep18 | Val MSE: 0.016937
#  Gate Ep19 | Val MSE: 0.015238
#  Gate Ep20 | Val MSE: 0.013405
#  Gate Ep21 | Val MSE: 0.011576
#  Gate Ep22 | Val MSE: 0.009968
#  Gate Ep23 | Val MSE: 0.008754
#  Gate Ep24 | Val MSE: 0.007929
#  Gate Ep25 | Val MSE: 0.007490
#  Gate Ep26 | Val MSE: 0.007291
#  Gate Ep27 | Val MSE: 0.007211
#  Gate Ep28 | Val MSE: 0.007183
#  Gate Ep29 | Val MSE: 0.007176
#  Gate Ep30 | Val MSE: 0.007177
#  Gate Ep31 | Val MSE: 0.007179
#  Gate Ep32 | Val MSE: 0.007182
#  Gate Ep33 | Val MSE: 0.007184
#  Gate Ep34 | Val MSE: 0.007185
#  Gate Ep35 | Val MSE: 0.007186
#  Gate Ep36 | Val MSE: 0.007187
#  Gate Ep37 | Val MSE: 0.007188
#  Gate Ep38 | Val MSE: 0.007190
#  Gate Ep39 | Val MSE: 0.007191
#  Gate Ep40 | Val MSE: 0.007192
#  Early stopping gating.
# ## Total MoE epochs: 190
# MoE Test - MSE (scaled): 0.009635, DirAcc(level): 0.8519, RMSE(passengers): 29.3490, MAE(passengers): 25.6559
# Training baseline (matched budget)...
#  BL Ep 20 | Val MSE: 0.018892
#  BL Ep 40 | Val MSE: 0.022500
#  BL Ep 60 | Val MSE: 0.018001
#  BL Ep 80 | Val MSE: 0.014688
#  BL Ep100 | Val MSE: 0.029888
#  BL Ep120 | Val MSE: 0.025435
#  BL Ep140 | Val MSE: 0.017602
#  BL Ep160 | Val MSE: 0.012772
#  BL Ep180 | Val MSE: 0.025425
#  BL Ep190 | Val MSE: 0.011202
# Baseline Test - MSE (scaled): 0.030372, DirAcc(level): 0.8148, RMSE(passengers): 39.9879, MAE(passengers): 35.1560


"""
Hard-sample mining and clustering on the Airline Passengers dataset
"""
...
class HParams:
    # features
    max_lag: int = 12  # monthly: one seasonal cycle of lags
...
# ---------------------------
# Data (Airline Passengers via sktime)
# ---------------------------

def load_airline_df() -> pd.DataFrame:
    """
    Returns a monthly DatetimeIndex DataFrame with a single column 'y' (passengers).
    sktime.datasets.load_airline returns a pandas Series with a PeriodIndex or DatetimeIndex.
    """
    y = load_airline()  # pandas Series
    # Ensure a proper monthly DatetimeIndex at month start
    idx = y.index
    if isinstance(idx, pd.PeriodIndex):
        dt = idx.to_timestamp(how="start")
    else:
        dt = pd.to_datetime(idx)
        # coerce to monthly-start frequency if needed
        dt = pd.DatetimeIndex(dt).to_period("M").to_timestamp(how="start")
    s = pd.Series(y.values.astype(float), index=dt).sort_index()
    # fill any tiny gaps via time interpolation (should be none, but safe)
    s = s.asfreq("MS").interpolate(method="time")
    return s.to_frame(name="y")

def build_features_from_df(df: pd.DataFrame, max_lag=12):
    """
    Features (leak-free):
      - y_lag1..y_lagN        (first max_lag columns form the LSTM gate sequence)
      - y_roll3_std           (shifted)
      - cyclical month: sin_month, cos_month (calendar vars; no shift needed)
    Target: y level (passengers)
    Returns: X, y, seq_len, feat_dim
    """
    if "y" not in df.columns:
        raise ValueError("Expected 'y' column in the airline dataset.")
    ...
    # Shifted rolling std (no leakage)
    df["y_roll3_std"] = df["y"].shift(1).rolling(3).std()

    df = df.dropna()

    # First max_lag features must be the y lags (for LSTM gate)
    feature_cols = [f"y_lag{lag}" for lag in range(1, max_lag + 1)]
    feature_cols += ["y_roll3_std", "sin_month", "cos_month"]
...
class GatingNetwork(nn.Module):
    """
    LSTM reads the y-lag slice (first max_lag columns) and outputs softmax weights over experts.
    """
...
def direction_acc_level(predict_fn, loader, lag_idx=0):
    """
    Direction on *level* (monthly): sign(ŷ_t - y_{t-1}) vs sign(y_t - y_{t-1}).
    Uses the first lag feature (y_lag1) at index 0.
    """
    correct, total = 0, 0
    for Xb, yb in loader:
        Xb, yb = Xb.to(device), yb.to(device)
        pred = predict_fn(Xb).squeeze(1)
        prev = Xb[:, lag_idx]  # y_lag1 at index 0
...
def moe_predict(gate, experts, xb, seq_len):
    ...
    w = gate(xb[:, :seq_len])                         # gate uses first seq_len = max_lag features (y lags)
...
def main():
    ...
    # Airline passengers -> features (programmatic load via sktime)
    df = load_airline_df()  # DatetimeIndex + 'y'
    ...
    print(f"MoE Test - MSE (scaled): {moe_mse_scaled:.6f}, DirAcc(level): {moe_dir:.4f}, "
          f"RMSE(passengers): {moe_rmse:.4f}, MAE(passengers): {moe_mae:.4f}")
    ...
    print(f"Baseline Test - MSE (scaled): {base_mse_scaled:.6f}, DirAcc(level): {base_dir:.4f}, "
          f"RMSE(passengers): {base_rmse:.4f}, MAE(passengers): {base_mae:.4f}")
