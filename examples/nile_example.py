...
#  Gen Ep 10 | Train MSE: 0.032318 | Val MSE: 0.020198
#  Gen Ep 20 | Train MSE: 0.029747 | Val MSE: 0.022774
#  Gen Ep 30 | Train MSE: 0.033082 | Val MSE: 0.019123
#  Gen Ep 40 | Train MSE: 0.029230 | Val MSE: 0.021819
# Clustering 19 hard samples into 3 specialists...
# Training specialists...
#  Specialist 1/3 trained on 6 samples.
#  Specialist 2/3 trained on 8 samples.
#  Specialist 3/3 trained on 5 samples.
# Training gating network...
#  Gate Ep 1 | Val MSE: 0.014811
#  Gate Ep 2 | Val MSE: 0.014805
#  Gate Ep 3 | Val MSE: 0.014799
#  Gate Ep 4 | Val MSE: 0.014795
#  Gate Ep 5 | Val MSE: 0.014790
#  Gate Ep 6 | Val MSE: 0.014787
#  Gate Ep 7 | Val MSE: 0.014784
#  Gate Ep 8 | Val MSE: 0.014781
#  Gate Ep 9 | Val MSE: 0.014779
#  Gate Ep10 | Val MSE: 0.014777
#  Gate Ep11 | Val MSE: 0.014776
#  Gate Ep12 | Val MSE: 0.014775
#  Gate Ep13 | Val MSE: 0.014774
#  Gate Ep14 | Val MSE: 0.014774
#  Gate Ep15 | Val MSE: 0.014774
#  Gate Ep16 | Val MSE: 0.014775
#  Gate Ep17 | Val MSE: 0.014776
#  Gate Ep18 | Val MSE: 0.014777
#  Gate Ep19 | Val MSE: 0.014779
#  Gate Ep20 | Val MSE: 0.014781
#  Gate Ep21 | Val MSE: 0.014783
#  Gate Ep22 | Val MSE: 0.014786
#  Gate Ep23 | Val MSE: 0.014789
#  Early stopping gating.
# ## Total MoE epochs: 138
# MoE Test - MSE (scaled): 0.017694, DirAcc(level): 0.7000, RMSE(volume): 121.5780, MAE(volume): 96.5683
# Training baseline (matched budget)...
#  BL Ep 20 | Val MSE: 0.025845
#  BL Ep 40 | Val MSE: 0.015966
#  BL Ep 60 | Val MSE: 0.019487
#  BL Ep 80 | Val MSE: 0.020025
#  BL Ep100 | Val MSE: 0.018462
#  BL Ep120 | Val MSE: 0.016577
#  BL Ep138 | Val MSE: 0.014850
# Baseline Test - MSE (scaled): 0.024640, DirAcc(level): 0.7000, RMSE(volume): 157.0981, MAE(volume): 134.6385


"""
Hard-sample mining and clustering on the Nile River flow dataset
"""
...
class HParams:
    # features
    max_lag: int = 3  # annual data; a few lags are enough
...
# ---------------------------
# Data (in-memory Nile, native)
# ---------------------------

def load_nile_df() -> pd.DataFrame:
    """
    Return a DataFrame with:
      - DatetimeIndex (yearly starts)
      - single numeric column: 'volume' (annual flow of the Nile)
    """
    d = sm.datasets.nile.load_pandas().data  # columns likely: 'year', 'volume'
    # Build datetime index from 'year'
    years = pd.to_datetime(d["year"].astype(int), format="%Y")
    df = pd.DataFrame({"volume": d["volume"].astype(float).values}, index=years)
    # Ensure annual frequency (fill rare gaps smoothly if any)
    s = df["volume"].resample("YS").mean().interpolate(method="time")
    return s.to_frame(name="volume")

def build_features_from_df(df: pd.DataFrame, max_lag=3):
    """
    Features (leak-free):
      - volume_lag1..lagN   (first max_lag columns feed the LSTM gate)
      - volume_roll3_std    (shifted by 1 to avoid leakage)
    Target: 'volume' level
    Returns: X, y, seq_len, feat_dim
    """
    if "volume" not in df.columns:
        raise ValueError("Expected a 'volume' column in the Nile dataset.")
    ...
    df["volume_roll3_std"] = df["volume"].shift(1).rolling(3).std()

    df = df.dropna()

    # First max_lag must be the lag features (for LSTM gate slice)
    feature_cols = [f"volume_lag{lag}" for lag in range(1, max_lag + 1)]
    feature_cols += ["volume_roll3_std"]
...
def direction_acc_level(predict_fn, loader, lag_idx=0):
    """
    Direction on level (annual): sign(ŷ_t - x_{t,lag1}) vs sign(y_t - x_{t,lag1}),
    where x_{t,lag1} is the first lag feature (volume_{t-1}).
    """
    correct, total = 0, 0
    for Xb, yb in loader:
        Xb, yb = Xb.to(device), yb.to(device)
        pred = predict_fn(Xb).squeeze(1)
        prev = Xb[:, lag_idx]  # volume_lag1 at index 0
...
def moe_predict(gate, experts, xb, seq_len):
    ...
    w = gate(xb[:, :seq_len])                         # gate uses first seq_len = max_lag features (volume lags)
...
def main():
    ...
    # Nile -> features (native, no files)
    df = load_nile_df()  # DatetimeIndex + 'volume'
    ...
    print(f"MoE Test - MSE (scaled): {moe_mse_scaled:.6f}, DirAcc(level): {moe_dir:.4f}, "
          f"RMSE(volume): {moe_rmse:.4f}, MAE(volume): {moe_mae:.4f}")
    ...
    print(f"Baseline Test - MSE (scaled): {base_mse_scaled:.6f}, DirAcc(level): {base_dir:.4f}, "
          f"RMSE(volume): {base_rmse:.4f}, MAE(volume): {base_mae:.4f}")
