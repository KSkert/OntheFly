...
#  Gen Ep 10 | Train MSE: 0.013514 | Val MSE: 0.012480
#  Gen Ep 20 | Train MSE: 0.013056 | Val MSE: 0.010356
#  Gen Ep 30 | Train MSE: 0.012305 | Val MSE: 0.008957
#  Gen Ep 40 | Train MSE: 0.013551 | Val MSE: 0.009844
# Clustering 49 hard samples into 3 specialists...
# Training specialists...
#  Specialist 1/3 trained on 25 samples.
#  Specialist 2/3 trained on 12 samples.
#  Specialist 3/3 trained on 12 samples.
# Training gating network...
#  Gate Ep 1 | Val MSE: 0.026231
#  Gate Ep 2 | Val MSE: 0.025318
#  Gate Ep 3 | Val MSE: 0.024400
#  Gate Ep 4 | Val MSE: 0.023467
#  Gate Ep 5 | Val MSE: 0.022449
#  Gate Ep 6 | Val MSE: 0.021275
#  Gate Ep 7 | Val MSE: 0.019893
#  Gate Ep 8 | Val MSE: 0.018249
#  Gate Ep 9 | Val MSE: 0.016315
#  Gate Ep10 | Val MSE: 0.014152
#  Gate Ep11 | Val MSE: 0.011961
#  Gate Ep12 | Val MSE: 0.010114
#  Gate Ep13 | Val MSE: 0.008750
#  Gate Ep14 | Val MSE: 0.008030
#  Gate Ep15 | Val MSE: 0.007644
#  Gate Ep16 | Val MSE: 0.007476
#  Gate Ep17 | Val MSE: 0.007405
#  Gate Ep18 | Val MSE: 0.007371
#  Gate Ep19 | Val MSE: 0.007359
#  Gate Ep20 | Val MSE: 0.007351
#  Gate Ep21 | Val MSE: 0.007342
#  Gate Ep22 | Val MSE: 0.007333
#  Gate Ep23 | Val MSE: 0.007329
#  Gate Ep24 | Val MSE: 0.007328
#  Gate Ep25 | Val MSE: 0.007328
#  Gate Ep26 | Val MSE: 0.007325
#  Gate Ep27 | Val MSE: 0.007322
#  Gate Ep28 | Val MSE: 0.007319
#  Gate Ep29 | Val MSE: 0.007312
#  Gate Ep30 | Val MSE: 0.007311
#  Gate Ep31 | Val MSE: 0.007308
#  Gate Ep32 | Val MSE: 0.007302
#  Gate Ep33 | Val MSE: 0.007300
#  Gate Ep34 | Val MSE: 0.007298
#  Gate Ep35 | Val MSE: 0.007297
#  Gate Ep36 | Val MSE: 0.007297
#  Gate Ep37 | Val MSE: 0.007298
#  Gate Ep38 | Val MSE: 0.007299
#  Gate Ep39 | Val MSE: 0.007301
#  Gate Ep40 | Val MSE: 0.007301
#  Gate Ep41 | Val MSE: 0.007303
#  Gate Ep42 | Val MSE: 0.007302
#  Gate Ep43 | Val MSE: 0.007302
#  Gate Ep44 | Val MSE: 0.007302
#  Gate Ep45 | Val MSE: 0.007301
#  Early stopping gating.
# ## Total MoE epochs: 160
# MoE Test - MSE (scaled): 0.022330, DirAcc: 0.9032, RMSE: 23.0721, MAE: 16.7610
# Training baseline (matched budget)...
#  BL Ep 20 | Val MSE: 0.012773
#  BL Ep 40 | Val MSE: 0.012596
#  BL Ep 60 | Val MSE: 0.011015
#  BL Ep 80 | Val MSE: 0.009121
#  BL Ep100 | Val MSE: 0.009967
#  BL Ep120 | Val MSE: 0.006205
#  BL Ep140 | Val MSE: 0.009127
#  BL Ep160 | Val MSE: 0.008435
# Baseline Test - MSE (scaled): 0.022841, DirAcc: 0.8548, RMSE: 24.0856, MAE: 17.6891


"""
Hard-sample mining and clustering on the Sunspots dataset
"""
...
# ---------------------------
# Data (in-memory Sunspots, native)
# ---------------------------

def load_sunspots_df() -> pd.DataFrame:
    """
    Return a DataFrame with:
      - DatetimeIndex (yearly)
      - single numeric column: 'sunspots'
    No file I/O, no internet.
    """
    d = sm.datasets.sunspots.load_pandas().data  # columns: YEAR, SUNACTIVITY
    years = pd.to_datetime(d["YEAR"].astype(int), format="%Y")
    df = pd.DataFrame({"sunspots": d["SUNACTIVITY"].astype(float).values}, index=years)
    # Ensure regular annual frequency (fill rare gaps smoothly if any)
    s = df["sunspots"].resample("YS").mean().interpolate(method="time")
    return s.to_frame(name="sunspots")

def build_features_from_df(df: pd.DataFrame, max_lag=3):
    """
    Create leak-free features from a DataFrame with:
      - DatetimeIndex (annual)
      - column 'sunspots'
    Features:
      - sunspots_lag1..sunspots_lagN
      - sunspots_roll3_std (shifted by 1 to avoid leakage)
    Returns: X, y, seq_len, feat_dim
    """
    if "sunspots" not in df.columns:
        raise ValueError("Expected a 'sunspots' column in the input DataFrame.")

    df = df.sort_index().copy()

    # Lags of sunspots
    for lag in range(1, max_lag + 1):
        df[f"sunspots_lag{lag}"] = df["sunspots"].shift(lag)

    # Shifted rolling std (uses data up to t-1)
    df["sunspots_roll3_std"] = df["sunspots"].shift(1).rolling(3).std()

    df = df.dropna()

    feature_cols = [f"sunspots_lag{lag}" for lag in range(1, max_lag + 1)] + ["sunspots_roll3_std"]
...
def direction_acc(predict_fn, loader, lag_idx=0):
    ...
    for Xb, yb in loader:
        ...
        prev = Xb[:, lag_idx]  # sunspots_lag1 is feature index 0
...
def main():
    ...
    # Sunspots -> features (native, no files)
    df = load_sunspots_df()  # DatetimeIndex + 'sunspots'
